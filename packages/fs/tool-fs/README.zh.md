...
当 `ctx.fs.sandboxMode` 表明提供方施加沙箱限制时，write/edit 会公开 `sandbox_permissions` 与 `justification`，并通过 `ctx.approval` 处理获批后的重试。若有效目标权限已被常驻模式覆盖，则该权限及理由作为冗余调用元数据被忽略；只有严格拓宽权限的请求才要求非空理由和审批。策略归属方会贡献与具体能力无关的常驻策略；工具结果仍保留针对具体操作的拒绝与重试引导。
