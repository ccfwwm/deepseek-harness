...
When `ctx.fs.sandboxMode` reports confinement, write/edit advertise `sandbox_permissions` and `justification` and resolve approved retries through `ctx.approval`. A valid target already covered by the standing mode is ignored as redundant call metadata, including its justification; only a strictly wider request requires a non-empty reason and approval. The policy owner contributes capability-neutral standing policy; the tool results retain operation-specific denial and retry guidance.
