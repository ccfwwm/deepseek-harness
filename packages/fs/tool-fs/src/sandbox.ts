...
import { ESCALATION_TARGETS, approveEscalation, escalationHintMarker, isSandboxPermissionCovered, sandboxDenialMarker, validateEscalationArgs } from '@deepseek-ai/dsh-sandbox'
...
export class FsSandboxController {
  ...
  async resolvePolicy(toolName: string, args: FsEscalationArgs, exec: ToolExecution): Promise<SandboxExecutionPolicy | undefined> {
    ...
    if (standingPolicy !== undefined && isSandboxPermissionCovered(standingPolicy.mode, args.sandbox_permissions)) {
