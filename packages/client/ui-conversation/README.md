...
ZeroWall compositions may provide `remote.zerowallFiles.prepareNative` to parse a file after its native upload completes. The draft becomes ready after parsing settles, retains its Agent-bound receipt, and remains sendable when the optional parser fails.
