...
type PreparedFile = NonNullable<ComposerFileAttachment['prepared']>
interface PreparedFileExtraction {
  kind: 'local' | 'mineru'
...
interface FilesRemote {
  ...
  inspect(input: {
    ...
    kind?: 'local' | 'mineru'
  }): Promise<RemoteResult<PreparedFile>>
  extract(input: {
    sessionId: SessionId
    attachmentId: string
    mode: 'local' | 'auto' | 'mineru'
  }): Promise<RemoteResult<PreparedFileExtraction>>
}
...
export class ConversationController extends Service implements IConversation {
  ...
  private async prepareDraftFile(
    ...
    try {
      const response = await remote.prepare({
        ...
        data: await base64ImageOf(attachment.file),
      })
      ...
      if (live?.kind === 'file') {
        live.prepared = { ...response.value, parseStatus: 'running', parseProgress: 12 }
        shell?.setDraft(shell.state.getSnapshot().draft)
      }
      ...
      if (!extraction.ok) throw new Error(`${extraction.error.code}: ${extraction.error.message}`)
      if (extraction.value.state !== 'done') {
        throw new Error(extraction.value.error ?? '附件解析失败')
      }
      const parsed = await remote.inspect({
        sessionId,
        attachmentId: response.value.attachmentId,
        view: 'parsed',
        kind: extraction.value.kind,
      })
      ...
      if (live?.kind === 'file') {
        ...
        if (live.prepared !== undefined) live.prepared = { ...live.prepared, storageStatus: 'failed' }
        shell?.setDraft(shell.state.getSnapshot().draft)
  ...
  async addImageBytesToDraft(sessionId: SessionId, input: {
    ...
    const drafts = this.createDrafts(sessionId, [file])
    const target = this.input.for(binding.ctx)
    if (!target.addAttachments(drafts.map(draft => draft.id))) {
