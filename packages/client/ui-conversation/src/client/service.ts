...
interface FilesRemote {
  prepare(input: { sessionId: string; name: string; mediaType?: string; data: string }): Promise<
...
export class ConversationController extends Service implements IConversation {
  ...
  async createDraftFiles(sessionId: SessionId, files: readonly File[]): Promise<readonly ComposerAttachment[]> {
