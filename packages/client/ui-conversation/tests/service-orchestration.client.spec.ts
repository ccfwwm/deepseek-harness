...
describe('ConversationController', () => {
  ...
  it('adds capability-owned image bytes and context to a session draft without sending', async () => {
    const b = await bench()
    const created = vi.spyOn(URL, 'createObjectURL').mockReturnValue('blob:presentation-slide')
    try {
      b.shell.setDraft('Please revise the colors')
      await b.root.addImageBytesToDraft(b.runtime.sessions.binding('s1')!.sessionId, {
        data: Uint8Array.of(1, 2, 3),
        mediaType: 'image/png',
        name: 'slide-04.png',
        contextText: 'presentationId=ppt-1, slideId=slide-4, page=4',
      })
      expect(b.shell.state.getSnapshot().imageIds).toHaveLength(1)
      expect(b.shell.state.getSnapshot().draft)
        .toBe('Please revise the colors\n\npresentationId=ppt-1, slideId=slide-4, page=4')
