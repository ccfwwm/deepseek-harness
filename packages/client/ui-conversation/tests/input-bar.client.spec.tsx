...
describe('image draft rail', () => {
  ...
  it('routes ordinary files through the attachment intake callback', () => {
    const addFiles = vi.fn(() => null)
    const result = bench({ addFiles })
    const file = new File(['notes'], 'notes.txt', { type: 'text/plain' })
    act(() => { attachmentOwner(result.slotCalls).onAddFiles([file]) })
    expect(addFiles).toHaveBeenCalledWith([file])
  })

  it('exposes an unrestricted attachment picker and preserves mixed selection order', () => {
    const addFiles = vi.fn(() => null)
    const result = bench({ addFiles })
  ...
  it('announces a file-service rejection as a fading toast, repeatable for the same reason', async () => {
    vi.useFakeTimers()
    try {
      const addFiles = vi.fn(() => '文件读取服务不可用')
