...
function dedupeAssemblyInputs(
  ...
  return {
    sections: uniqueSections.length === sections.length ? sections : uniqueSections,
    tools: uniqueTools.length === tools.length ? tools : uniqueTools,
  }
}
  ...
  constructor(ctx: Context, config: Config) {
    ...
    if (config.includeHarnessIdentity ?? true) {
      this.section({
        ...
        text: 'You are an AI agent running inside ZeroWall Science Desktop.',
      })
    }
    this.section({
