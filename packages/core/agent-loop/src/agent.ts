...
export class ReactLoopAgent implements Agent {
  ...
  private buildRequest(
    ...
    this.loopCtx.logger.info(
      `agent request metrics: session=${String(this.session.id)} turn=${this.phase.kind === 'running' ? this.phase.turn : 0} step=${this.phase.kind === 'running' ? this.phase.step : 0}`
      + ` surfaceGeneration=${surfaceGeneration}`
      + ` systemBytes=${utf8Bytes(boundaryMessages.filter(message => message.role === 'system'))}`
